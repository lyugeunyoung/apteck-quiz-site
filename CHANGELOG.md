...
- **정보 구조 재배열**: 문제(1) → **정답(2, 클릭 없이 바로 노출)** → 해설(3, 자세한 설명은 계속 펼치기로
  유지) → 참여방법(4) → 지난정답(5) → FAQ(6) → 관련퀴즈(7). 기존엔 정답 요약 카드가 목차 위에 번호 없이
  떠 있었는데, 이제 목차 2번 섹션으로 편입돼 문제 바로 아래에서 자연스럽게 이어진다.
  광고 영역 B도 새 "정답" 섹션 바로 아래로 이동.
- **문제 이미지가 있으면 텍스트 보기 생략**: 실제 앱 스크린샷에 보기(①②③④)가 이미 나와 있는 경우,
  같은 내용을 텍스트로 또 나열하지 않는다. 이미지가 없을 때만(문제만 등록되고 이미지가 아직 없는 경우)
  텍스트 보기가 유일한 정보원이므로 그대로 보여준다.
- 회차 앱(카카오뱅크 AI 이모지 퀴즈)도 같은 구조로 통일 — `renderRoundBlock`을 "문제 전용"/"정답·해설
  전용" 두 함수로 분리해 회차별 문제 섹션과 회차별 정답 섹션을 나눴다.
- 참고: bookshelf-journey.tistory.com 디자인을 참고해 페이지마다 다르게 뜨던 "관련 퀴즈" 추천을 모든
  페이지가 같은 "오늘의 픽" 목록을 쓰도록 통일한 것(직전 커밋)에 이어지는 후속 요청.
