...
## Phase 7 이후 — 상세 페이지 히어로/이미지/정답 디자인 개선 + "오늘의 픽" 수동 선택 (2026-09-15)

- **히어로 영역 재구성**: 썸네일(원본 300×300, 화면엔 72px로 표시)과 카테고리·제목을 한 줄에
  나란히 묶어 아이콘과 제목의 연결감을 살렸다(기존엔 뱃지 56px + 즐겨찾기만 한 줄, 제목은 아래
  별도 줄). `.quiz-hero__top`/`.quiz-hero__identity`/`.quiz-hero__titlewrap` 신설.
- **문제 이미지 크기 버그 수정**: `.q-image img`가 `width:100%`라 데스크톱처럼 패널이 원본보다
  넓으면 이미지가 원본 해상도 이상으로 확대돼 흐려졌다. `width:auto; max-width:100%`로 바꿔
  원본 해상도(예: 400×790)를 넘지 않으면서, 모바일처럼 화면이 좁을 때만 컨테이너 폭에 맞춰
  축소되도록 고쳤다(데스크톱 401×791 실측 확인, 모바일 375px 뷰포트에서도 좌우 스크롤 없이
  비율 유지된 축소 확인).
- **목차(TOC) 글자 크기 확대**: 12.5px/500 → 14.5px/700, 패딩도 넓혀 전체 화면 대비 너무 작아
  보이던 문제를 고쳤다.
- **"오늘의 정답" 강조 디자인**: 그라디언트 배경(브랜드 컬러 반영) + 2px 테두리 + 그림자 + 더 큰
  글자로 스크롤 없이도 정답이 바로 눈에 들어오게 했다(단일 회차 `.answer-summary__value`,
  다회차 `.answer-summary__round` 둘 다 동일하게 적용).
- **"오늘의 픽" 관리자 수동 선택**: 지금까지는 앱 목록 순서 그대로 앞쪽 3개를 자동으로 골랐는데,
  이제 `admin.html` "4. 사이트 전역 설정"에서 정확히 3개를 직접 지정할 수 있다
  (`site.featuredAppIds`). `pickFeatured()`가 지정값을 최우선으로 쓰고, 3개 미만이면 기존
  자동 알고리즘으로 나머지를 채운다 — 비워두면 완전히 이전과 동일하게 동작(하위 호환).
- **SEO/AEO 점검**: FAQPage·Article(datePublished/dateModified/speakable)·BreadcrumbList
  JSON-LD, OG/Twitter 카드, canonical, `robots.txt`+`noindex`로 admin 차단, sitemap/feed/llms.txt가
  이미 Phase 4~7에서 갖춰져 있음을 재확인 — `npm test`(`scripts/verify.js`) 9/10 PASS(1개는
  소유확인 코드 미등록으로 스킵) 유지. 이번 변경분에서 새로 발견된 구조적 격차는 없었다.
- **공유 이미지 재확인**: 사용자가 재전달한 하나원큐 썸네일/문제/coming-soon 3개 구글드라이브
  이미지를 다시 내려받아 바이트 수·해상도를 비교한 결과 기존 반영본과 동일 — 이미지 자체는
  변경되지 않았고(설정 로직만 재확인 요청으로 해석), coming-soon 치환 로직은 기존 그대로 정상
  동작.

## Phase 7 이후 — 앱 썸네일 이미지 지원 + 실제 문제 이미지 반영 (2026-09-15)

- **앱 썸네일(`app.thumbnailUrl`) 신설**: 이모지 대신 실제 브랜드 이미지를 아이콘으로 쓸 수 있게 했다.
  홈 카드·상세 페이지 히어로·"오늘의 픽" 카드·"전체 목록" pill 4곳 모두 `renderAppBadge()` 공용 함수로
  통일해, 값이 있으면 이미지를, 없으면 기존처럼 이모지를 보여준다(21개 중 나머지 앱은 그대로 이모지 —
  하위 호환).
- **하나원큐 축구Play 퀴즈HANA에 실제 이미지 2종 적용**: 사용자가 전달한 구글드라이브 이미지를 원본
  해상도 그대로 WebP로 로컬화해 반영 — 브랜드 썸네일(300×300)과 오늘 실제 문제 화면 캡처
  (400×790, 이미 시트로 동기화된 정답 "② 교토 상가"와 정확히 일치하는 문제 화면). 문제 이미지가
  생겨서 기존 로직대로 텍스트 보기 목록은 자동으로 생략됨.
- **버그 수정**: `.q-image img`에 `width: 100%`만 있고 `height: auto`가 없어서, 세로로 긴 문제
  스크린샷이 원본 비율 없이 세로 길이가 고정돼 가로로 찌그러져 보이는 문제를 발견해 수정.

## Phase 7 이후 — 실사용 구글 시트 형식 지원 (2026-09-15)

- **근본 원인**: 실제 운영 중인 구글 시트를 연결해보니 하나원큐 축구Play 퀴즈HANA를 포함해 아무 데이터도
  안 불러와졌다. 실제 시트를 직접 열어 확인한 결과, 우리 코드는 `app_id` 열로 앱을 찾도록 만들어져
  있었는데 실사용 시트는 `app_id` 열 자체가 없고 `"A (구분/ID)"` 열에 앱의 표시 이름만 적혀 있었다 —
  그래서 시트의 모든 행이 "app_id 없음"으로 검증에 실패해 일괄 반영 자체가 통째로 막혀 있었다(하나원큐
  행 자체는 멀쩡했는데도).
- **시트 형식 지원 확장**: `templates.normalizeSheetRow`/`resolveSheetApp`을 신설해
  `"A (구분/ID)"`, `"B (question)"` 같은 괄호 라벨 헤더와, `app_id` 없이 앱 이름(공백 차이 무시)으로
  찾는 방식을 함께 지원. 기존 단순 헤더(`date,app_id,question...`) 시트도 그대로 계속 동작(추가 방식이라
  기존 걸 안 건드림). 이름이 시트와 다르게 표기되는 앱(monimo 모니스쿨, 카카오페이 퀴즈타임)은
  `app.sheetName` 별칭 필드로 매칭.
- **검증 철학 변경**: "시트 행 하나가 앱과 매칭 안 됨"을 더는 전체 동기화를 막는 에러로 취급하지 않음
  (참고 로그만 남기고 그 행만 건너뜀) — 20여 개 앱이 한 시트에 섞여 있는 실사용 환경에서는 아직 등록
  안 된 앱 행이 항상 있을 수 있는데, 예전 방식대로면 그 행 하나가 나머지 모든 앱의 정상 반영을 영구히
  막을 수 있었다. 날짜(`date`) 열 자체가 없는 시트는 행 하나 = "지금 상태"로 취급(항상 반영 대상).
- **해설 리치 텍스트 지원**: 실사용 시트의 해설(E열)은 `<b>`, `<mark>`, `<br />` 같은 실제 HTML 서식으로
  들어온다 — 기존엔 무조건 이스케이프해 태그가 글자 그대로 깨져 보였다. `renderRichText()`를 신설해
  HTML처럼 보이면 그대로(위험한 태그만 방어적으로 제거) 렌더링하고, 순수 텍스트는 기존처럼 줄바꿈만
  처리한다.
- **부가 열 지원**: `D(link)`→참여 링크 자동 반영, `H(custom_title)`→페이지 제목 재정의,
  라벨 없는 I/J/K열→정답 대기 문구·홍보 문구·힌트로 각각 매핑해 렌더링에 반영.
- **`csvToObjects` 버그 수정**: 헤더가 비어있거나 겹치면(실사용 시트의 I/J/K열처럼) 같은 키로 값을
  덮어써 사라지는 문제를 열 문자(a/b/c...) 폴백으로 수정.
- **실제 반영**: 위 수정을 실제 시트로 검증(관리자 페이지 UI 경로 + 자동 동기화 스크립트 경로 둘 다) 후,
  1회성으로 21개 앱 중 20개에 실제 데이터를 반영(하나원큐 축구Play 퀴즈HANA 포함). 반복 자동 동기화는
  아직 켜지 않음(`site.sheetUrl` 비움) — admin.html "자동 동기화 켜기"는 사용자가 원할 때 직접 켜도록
  남겨둠. 시트에는 있지만 아직 사이트에 없는 앱 "비트버니 코인이 쌓이는 퀴즈"는 추가하지 않고 건너뜀.
