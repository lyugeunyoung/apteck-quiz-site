...
1. 구글 시트를 새로 만들고 첫 줄(헤더)에 아래 열 이름을 정확히 입력하세요 (순서는 상관없습니다).
   ...
   | date | app_id | question | choice1 | choice2 | choice3 | choice4 | answer | explanation | image_url |
   |---|---|---|---|---|---|---|---|---|---|
   | 2026-09-04 | toss-luck | 오늘 문제... | ① 서울 | ② 부산 | | | ① 서울 | 해설... | (선택) |

   - `date`는 `YYYY-MM-DD` 형식, `app_id`는 관리자 페이지 앱 목록에서 해당 앱을 선택했을 때
     `data/quizzes.json`에 저장된 영문 ID와 정확히 같아야 매칭됩니다 (예: `toss-luck`, `kakaopay-quiztime`).
   - 객관식이 아니면 `choice1`~`4`는 비워두세요. `image_url`도 선택 항목입니다.
   - 한 행 = 한 앱의 하루 치 데이터입니다. 앱마다, 날짜마다 새 행을 추가하세요.
...
3. 아래 형식으로 CSV 주소를 만드세요. `시트ID`는 시트 URL의 `/d/`와 `/edit` 사이 문자열,
   ...
   ```
4. 관리자 페이지의 **"2. 구글 시트에서 자동 불러오기"** 칸에 이 주소를 붙여넣고 **시트 불러오기** 클릭.
5. 이후 앱 목록에서 앱을 고를 때마다, 그 앱의 오늘 날짜 행이 시트에 있으면
   **"📄 시트에서 오늘 행을 찾았습니다 — 이 값으로 채우기"** 배너가 떠서 클릭 한 번으로 문제·보기·정답·해설·
   이미지가 자동으로 채워집니다. 오늘 날짜 행이 없으면 그 앱의 가장 최근 행을 대신 찾아 보여줍니다.
   채워진 뒤에도 저장 전에는 자유롭게 수정할 수 있고, 실제로 GitHub에 반영되는 건 평소처럼
   **GitHub에 저장 & 게시**를 눌렀을 때뿐입니다 — 시트 불러오기는 입력을 도와줄 뿐 자동 게시는 하지 않습니다.
