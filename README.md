...
1. 코드 작업을 시작하기 전에 `git pull`로 최신 상태를 받으세요.
2. 작업을 마치면 바로 `git push`로 올려두세요 (다른 컴퓨터에서 계속하기 전에 받아갈 수 있도록).
...
### GitHub Actions 자동 동기화는 컴퓨터와 무관하게 항상 동작

구글 시트 자동 동기화(7번 섹션)는 GitHub의 서버에서 실행되므로, 어느 컴퓨터도 켜져 있을 필요가
없습니다. 시트에 값만 입력해두면 두 컴퓨터 중 어느 쪽도 안 열려 있어도 사이트가 알아서 갱신됩니다.

> 참고: 이 자동 동기화 워크플로는 페이지를 재생성할 때마다 상단 날짜 표시와 "오늘 업데이트/업데이트
> 대기" 상태를 그날 기준으로 다시 계산하기 때문에, 시트에 새 내용이 없어도 날짜가 바뀐 날이면 사소한
> 자동 커밋(`apteck-sheet-sync` 작성)이 하루 한 번 정도 생길 수 있습니다. 정상 동작이며 콘텐츠가
> 사라지거나 덮어써지는 것이 아니니 걱정하지 않아도 됩니다.
